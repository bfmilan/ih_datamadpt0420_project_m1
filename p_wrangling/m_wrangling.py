...
def rename_column(raw_data):
    ...
    raw_data.rename(columns={'country_code': 'Country', 'gender': 'Gender',
                 ...
                 inplace=True)
    raw_data.rename(columns={raw_data.columns[1]: "Quantity"})
    return raw_data
    print('All columns renamed!')
'''
def remove_multiples(raw_data, varname):
    """
    ...
    """
    if (varname in raw_data.columns):
        tmp = raw_data.iloc[:, min([i for i,x in enumerate(raw_data.columns == varname) if x])]
        del raw_data[varname]
        raw_data[varname] = tmp
    return raw_data



def df_stats(raw_data):
    print('Getting some column stats...')
    df_stats = pd.DataFrame()
    for (column,col) in {'Country':Country, 'Gender':Gender, 'Job Title': Job_Title, 'uuid':uuid}.items():
        total_nulls= col.isnull().sum().sum()
        df_stats.loc[column, 'total_nulls'] = total_nulls
        # df_stats.loc[column, 'total_columns'] = df.shape[1]
        df_stats.loc[column, 'total_rows'] = df.shape[0]
    print(df_stats)
'''

def wrangling(raw_data: object) -> object:
    jobs_column(raw_data)
    country_column(raw_data)
    rural_column(raw_data)
    gender_column(raw_data)
    rename_column(raw_data)
    # remove_multiples(raw_data,varname)
