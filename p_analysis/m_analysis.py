...
def challenge1(data, country='All'):
	...
	ch1.rename(columns={'uuid': 'Quantity'},
				  inplace=True)
	print(f'Before drop the shae is: {ch1.shape}')
	ch1 = ch1[~ch1['Job Title'].isin(['none'])]
	# ch1 = ch1.replace(to_replace='None', value=np.nan).dropna(inplace=True)
	print(f'After drop the none values the shape is: {ch1.shape}')
	...
	# new line to drop none
	ch1.drop(ch1.loc[ch1['Job Title'] == 'none'].index, inplace=True)

	# print(ch1.head(10), ch1.tail(5))
